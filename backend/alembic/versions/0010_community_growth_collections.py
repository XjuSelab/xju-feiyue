...
down_revision: str | None = "0012_class_missions"
...
def upgrade() -> None:
    ...
    # comments 用 batch（表重建）：SQLite 不支持 ALTER TABLE ADD CONSTRAINT，
    # 直接 op.create_foreign_key 会抛 NotImplementedError。0004 已有 batch 先例。
    with op.batch_alter_table("comments") as batch:
        batch.add_column(sa.Column("parent_id", sa.String(length=64), nullable=True))
        batch.add_column(sa.Column("reply_to_sid", sa.String(length=11), nullable=True))
        batch.add_column(
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("comments") as batch:
        batch.drop_constraint("fk_comments_reply_to_sid_users", type_="foreignkey")
        batch.drop_constraint("fk_comments_parent_id_comments", type_="foreignkey")
        batch.drop_column("status")
        batch.drop_column("images")
        batch.drop_column("reply_to_sid")
