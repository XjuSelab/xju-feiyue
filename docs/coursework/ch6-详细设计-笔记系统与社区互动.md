...
〔此处插入图 6-1 —— 源文件 figures/ch6-fig1-class-program.drawio，app.diagrams.net 编辑后导出插入〕
...
本模块主要软件界面如下（按功能域分组，规划项界面标注「规划」），原型见图 6-2～图 6-7。

### 4.1 写作页界面（/write）

上半部为 Markdown 编辑器（标题、分类、标签、正文输入区）并带草稿自动保存指示；正文选区浮出 AI 润色工具条（润色/缩写/扩写/改语气/翻译/自定义 + 差异对照面板，可整块采纳或拒绝）；右侧为 AI 一句话摘要区（SSE 流式展示 + 生成按钮，失败自动降级）；底部为发布按钮（发布前校验标题/正文/分类必填）。

〔此处插入图 6-2 —— 源文件 figures/ch6-fig2-ui-write.drawio〕

图 6-2 写作页界面（/write）

### 4.2 信息流浏览页界面（/browse）

左侧为分类与热门标签筛选侧栏；顶部为搜索栏与排序切换（最新/热门/我赞过）；中部为两列笔记卡片网格（封面、标题、一句话摘要、作者与互动计数）；底部为「加载更多」游标分页。

〔此处插入图 6-3 —— 源文件 figures/ch6-fig3-ui-browse.drawio〕

图 6-3 信息流浏览页界面（/browse）

### 4.3 笔记详情页界面（/note/:id）

左侧为正文区（Markdown 渲染、代码高亮、锚点引用高亮）与操作栏（点赞、评论显示模式切换、评论数）；右侧为评论区（锚点引用、平铺分页、创建与删除）。点踩、收藏、合集侧栏、评论九宫格图片、评论赞踩、举报/拉黑菜单为前端后续接入项（图中红色虚线标注）。

〔此处插入图 6-4 —— 源文件 figures/ch6-fig4-ui-detail.drawio〕

图 6-4 笔记详情页界面（/note/:id）

### 4.4 个人中心·合集管理界面（/me，规划）

后端已提供合集 CRUD 与条目管理接口，/me 合集 Tab 为前端后续接入。界面左侧为「我的合集」列表与新建入口，右侧为所选合集内笔记（封面、状态、移出、拖拽排序，遵循单篇笔记单归属校验）。

〔此处插入图 6-5 —— 源文件 figures/ch6-fig5-ui-collection.drawio〕

图 6-5 个人中心·合集管理界面（规划）

### 4.5 举报工单管理界面（/admin，规划）

当前管理后台未接入举报工单队列，下图为后续治理界面设计：左侧为工单队列（含状态 pending/ai_flagged/resolved），右侧为工单详情——目标内容快照、AI 审查结论（label/confidence/reason）、举报类型与人工裁决按钮（删除/隐藏、驳回恢复）。

〔此处插入图 6-6 —— 源文件 figures/ch6-fig6-ui-report.drawio〕

图 6-6 举报工单管理界面（规划）

### 4.6 每日签到弹窗界面

用户每日首次上线时以弹窗承载签到：展示本次经验 +5、连续签到天数与当前等级，提供一键签到按钮；当日已签到则不再弹出（幂等，不重复发放经验）。

〔此处插入图 6-7 —— 源文件 figures/ch6-fig7-ui-checkin.drawio〕
...
（图 6-8）用户点击发布 → 前端校验标题/正文/分类 → 调用 POST /notes/drafts/{id}/publish → 后端检验草稿 → 摘要为空时调用 AI → AI 超时/失败取首段截断 → 事务建笔记删草稿 → 返回新笔记 → 前端跳转详情页。

〔此处插入图 6-8 —— 源文件 figures/ch6-fig8-seq-publish.drawio〕

图 6-8 草稿发布与 AI 摘要降级顺序图

### 6.2 举报—AI 审查—人工裁决规划顺序图

（图 6-9）该链路为后续治理能力设计，当前代码尚未实现。规划流程为：用户点击举报 → 前端弹出举报面板选择类型 → POST /reports → 后端创建工单 → 返回"已受理" → 异步送 AI 审查 → AI 返回 {label, confidence, reason} → 高置信自动隐藏目标内容 → 管理员打开举报队列 → 查看工单 + AI 结论 → 裁决删除/恢复 → 更新工单状态。

〔此处插入图 6-9 —— 源文件 figures/ch6-fig9-seq-report.drawio〕

图 6-9 举报—AI 审查—人工裁决规划顺序图

### 6.3 合集上下文查询顺序图

（图 6-10）后端已实现合集上下文查询：前端或调试客户端请求 GET /notes/{id}/collection → 后端查询 collection_entries 并 JOIN notes/author → 排序组装 → 返回合集信息 + 笔记列表 + currentIndex；若无归属则返回 200/null。详情页侧栏展示与导航仍为前端待接入项。

〔此处插入图 6-10 —— 源文件 figures/ch6-fig10-seq-collection.drawio〕
...
（图 6-11）当前已实现的笔记状态迁移为：草稿（Draft）→ 发布（visible）→ 作者删除。pending 隐藏态、AI 高置信标记与管理员裁决为治理扩展规划。

〔此处插入图 6-11 —— 源文件 figures/ch6-fig11-state-note.drawio〕

图 6-11 笔记生命周期状态图

### 7.2 评论状态图

（图 6-12）当前已实现的评论状态迁移为：创建（visible）→ 评论作者或笔记作者删除。pending 隐藏态、AI 高置信标记与管理员裁决为治理扩展规划。

〔此处插入图 6-12 —— 源文件 figures/ch6-fig12-state-comment.drawio〕

图 6-12 评论状态图

### 7.3 举报工单状态图

（图 6-13）举报工单状态迁移为规划项：pending（待处理）→ ai_flagged（AI 已标记）→ resolved（已裁决删除/隐藏） / dismissed（驳回恢复）。

〔此处插入图 6-13 —— 源文件 figures/ch6-fig13-state-report.drawio〕
...
（图 6-14）用户选择分类 → 输入关键词 → 选择标签 → 切换排序 → 调用 GET /notes → 后端应用层过滤 → 游标分页 → 返回列表 → 前端渲染卡片 → 滚动触发"加载更多" → 更新游标 → 追加列表。

〔此处插入图 6-14 —— 源文件 figures/ch6-fig14-activity-browse.drawio〕

图 6-14 信息流筛选/排序/分页活动图

### 8.2 AI 流式摘要三层降级活动图

（图 6-15）触发生成 → SSE 流式接收 → 首个 chunk 到达即展示 → 逐字写入 → 完成。分支：Stream 失败 → 回退 one-shot → 成功则回填；失败 → mock 伪流式 → 生成静态文本。

〔此处插入图 6-15 —— 源文件 figures/ch6-fig15-activity-ai.drawio〕

图 6-15 AI 流式摘要三层降级活动图
