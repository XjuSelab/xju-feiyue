...
| 举报 | 未实现 | 当前仅有 reports 表模型，无 `/reports` 路由 |
| 拉黑 | 未实现 | 当前仅有 blocks 表模型，无 `/blocks` 路由 |
...
UML 类图对应的关系模式如下（下划线表示主键，波浪线表示外键）：
...
| E-AUTH-004 | 非管理员访问管理后台 | 404 | 返回不存在，隐藏管理入口 |
...
| E-REPORT-001 | 同一用户重复举报同一内容 | — | 规划项：实现举报接口时幂等合并到既有工单 |
...
3. **幂等保护**：
   - 点赞/点踩/收藏/签到以数据库唯一约束保证幂等；举报接口尚未实现
   - 客户端网络重试不会产生重复数据

4. **数据一致性**：
   - 发布走"草稿→笔记"单事务原子操作，不允许中间态
   - 删除笔记级联清除点赞、评论（数据库 ondelete=CASCADE）
   - 经验流水当前由每日签到写入；互动经验加减与取消扣回为后续扩展
